...
(function ($) {
   ...
   // ===== 16. About Us section Img mousemove 
    
        const btns = document.querySelectorAll(".mouseeffect");
        
    btns.forEach((mouseeffect) => {
        mouseeffect.addEventListener("mousemove", function(e){
            const position = mouseeffect.getBoundingClientRect();
            const x = e.pageX - position.left - position.width;
            const y = e.pageY - position.top - position.height;
                
            mouseeffect.children[0].style.transform = "translate(" + x * 0.2 +"px, " + y * -0 +"px)";
            });
        });
